...
static void LCD_Init_Backlight(void) {
	...
	PWM_Init(LCD_BL_TPM, LCD_BL_TPM_CHANNEL, LCD_BL_PERIOD, 3*LCD_BL_PERIOD/4);	
...
have called LCD_Write_Rectangle before calling this function. */
__forceinline void LCD_Write_Rectangle_Pixel_Components(uint8_t R, uint8_t G, uint8_t B) {
	uint8_t b1, b2;
...
/* Plot these colors in the next pixel location as defined by LCD_Start_Rectangle. You must 
have called LCD_Write_Rectangle before calling this function. */
void LCD_Write_Rectangle_Quad_Pixel_Components(uint32_t * aR, uint32_t * aG, uint32_t * aB) {
	uint8_t b1, b2, i;
	uint32_t R, G, B;
...
void LCD_Write_Rectangle_N_Quad_Pixel_Components(uint32_t * aR, uint32_t * aG, uint32_t * aB, int32_t n) {
	uint8_t b1, b2; 
#if 1
	uint8_t i;
#else
