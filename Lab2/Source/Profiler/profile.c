...
void Process_Profile_Sample(void) {
	...
	PC_val =
			*((unsigned int *) (CUR_SP + FRAME_SIZE + HW_RET_ADX_OFFSET));
...
void Sort_Profile_Regions(void) {
	...
	for (i = 0; i < NumProfileRegions; ++i) {
		for (j = i + 1; j < NumProfileRegions; ++j) {
			if (RegionCount[SortedRegions[i]] <
					RegionCount[SortedRegions[j]]) {
...
void Display_Profile(void) {
	...
	row = 0;
	p.Y = ROW_TO_Y(row);
	snprintf(buffer, LCD_MAX_COLS, "%6lu Tot. Samples",
					 profile_samples);
	...
	for (i = 0; i < NumProfileRegions; i++) {
		if (RegionCount[SortedRegions[i]] > 0) {
			p.X = COL_TO_X(0);
			p.Y = ROW_TO_Y(row);
			snprintf(buffer, LCD_MAX_COLS, "%4d %s",
							 RegionCount[SortedRegions[i]],
							 RegionTable[SortedRegions[i]].Name);
			LCD_Text_PrintStr(&p, buffer);
			row++;
			if (row >= LCD_MAX_ROWS) {
...
void New_Display_Profile(void) {
	...
	row = 0;
	p.Y = ROW_TO_Y(row);
	snprintf(buffer, LCD_MAX_COLS, "%6lu Total Samples",
					 profile_samples);
	LCD_Text_PrintStr(&p, buffer);
	row++;
	i = 0;
	...
	while (1) {
		for (row = 0; row < LCD_MAX_ROWS - 1; row++) {
			if ((i < NumProfileRegions) && (RegionCount[i] > 0)) {
				p.X = COL_TO_X(0);
				p.Y = ROW_TO_Y(row);
				snprintf(buffer, LCD_MAX_COLS, "%4d %s", RegionCount[i],
								 RegionTable[i].Name);
				LCD_Text_PrintStr(&p, buffer);
			}
			i++;
